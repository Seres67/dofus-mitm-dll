...
bool HookEngine::Hook(std::string_view moduleName, std::string_view functionName, HookFncPtr hookAddress) const
{
    ...
    m_hookedFunctions[std::string{ moduleName }].emplace_back(std::string{ functionName }, functionAddress);
...
bool HookEngine::Hook(FncPtr originalAddress, HookFncPtr hookAddress) const
{
...
bool HookEngine::Unhook(FncPtr originalFunction, HookFncPtr hookAddress) const
{
    DetourTransactionBegin();
    DetourUpdateThread(GetCurrentThread());
...
/*static*/ ModuleAddressPtr HookEngine::GetModuleAddress(std::string_view moduleName)
{
    ...
    const CHAR *moduleNameStr{moduleName.data() };
    auto newModuleAddress{ (ModuleAddressPtr)GetModuleHandle(moduleNameStr) };
    if (newModuleAddress == nullptr) {
        std::cerr << std::format("Could not get address of {}... Attempting to load...", moduleName)
                  << std::endl;
        newModuleAddress = (ModuleAddressPtr)LoadLibrary(moduleNameStr);
        if (newModuleAddress == nullptr) {
